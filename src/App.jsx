...
export default function ApniDukanApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Only seed the default catalog the very first time this store is
        // ever opened (no saved document yet). If a document already exists
  ...
  useEffect(() => {
    const unsubscribe = storageListen("products", (rawValue) => {
      try {
        ...
        // Accept the update even if the catalog is now empty (e.g. admin
        // deleted every product) — only skip a genuinely broken/undefined
        // payload, never a legitimate empty array.
  ...
  // Always read the latest saved product list right before mutating it —
  // never trust the local `products` state alone for writes. This prevents
  // the bug where a slightly-stale local copy (e.g. from a race between two
  // quick admin actions, or another open tab) gets saved back and silently
  // resurrects a product that was just deleted.
  async function getFreshProducts() {
    try {
      const res = await storageGet("products");
      return res ? JSON.parse(res.value) : products;
    } catch {
      return products;
    }
  }
  ...
  async function deleteAllProductsInCategory() {
    ...
    const fresh = await getFreshProducts();
    const next = fresh.filter((p) => p.category !== deleteCategoryName);
    const removedCount = fresh.length - next.length;
    setProducts(next);
  ...
  .phone-frame{width:100% !important;height:100vh !important;max-height:none !important;border:none !important;border-radius:0 !important;box-shadow:none !important;}
                  ...
                  <div style={styles.cardImgWrap}>
                    {resolveProductImage(p) ? (
                      <img
                        ...
                        alt={p.name}
                        style={{ ...styles.cardImg, cursor: "zoom-in" }}
                        onClick={(e) => { e.stopPropagation(); setZoomImage({ src: resolveProductImage(p), name: p.name }); }}
                      />
                    ) : (
      ...
      {zoomImage && (
        <div
          ...
          style={{
            position: "absolute", inset: 0, background: "rgba(20,15,10,0.88)",
            display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
            zIndex: 50, padding: 20, cursor: "zoom-out",
          }}
        >
          <img
            src={zoomImage.src}
            alt={zoomImage.name}
            style={{
              maxWidth: "100%", maxHeight: "78%", borderRadius: 14,
              boxShadow: "0 20px 50px rgba(0,0,0,0.5)", objectFit: "contain",
              animation: "zoomPop 0.22s ease-out",
            }}
          />
          <div style={{ color: "#fff", marginTop: 14, fontWeight: 700, fontSize: 15, textAlign: "center" }}>{zoomImage.name}</div>
          <button
